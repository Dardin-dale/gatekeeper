...
// `world` option choices: the configured worlds for the active game, so the
// picker offers them instead of asking players to remember exact spellings
// (`Emmumóðir` is not a thing anyone types twice). Read from the same
// config/<game>.worlds.json the stack reads into WORLDS_JSON, so the picker and
// the bot's actual world list are the same source — a world the Lambda doesn't
// know about could never be started anyway.
//
// Registration-time, not autocomplete: the world list IS deploy-time (it ships as
// a Lambda env var), and `postdeploy` re-registers commands on every deploy, so
// the picker updates itself whenever the worlds do. Only the NAME is used — this
// schema is public, and the config also holds per-world passwords.
const WORLD_CHOICES = (() => {
  try {
    const fs = require('fs');
    const path = require('path');
    const gameId = (GAME && GAME.id) || process.env.GAME || 'abiotic-factor';
    const file = path.join(__dirname, '../config', `${gameId}.worlds.json`);
    if (!fs.existsSync(file)) return null;
    const worlds = JSON.parse(fs.readFileSync(file, 'utf8'));
    if (!Array.isArray(worlds) || worlds.length === 0) return null;
    // Discord rejects a command with >25 choices; fall back to free text rather
    // than failing registration for a deployment with a lot of worlds.
    if (worlds.length > 25) return null;
    return worlds
      .filter((w) => w && typeof w.name === 'string' && w.name)
      .map((w) => ({ name: w.default ? `${w.name} (default)` : w.name, value: w.name }));
  } catch (e) {
    return null; // never let a malformed config block command registration
  }
})();

// Attach the choices to a `world` option, or leave it free-text when there are
// none to offer. Choices RESTRICT input to the listed values, which is what we
// want: an unlisted world can't be started.
function worldOption(description) {
  return {
    type: STRING,
    name: 'world',
    description,
    required: false,
    ...(WORLD_CHOICES ? { choices: WORLD_CHOICES } : {}),
  };
}
...
const DISCORD_COMMANDS = [
  {
    ...
    options: [
      ...
      },
      {
        ...
        options: [
          worldOption("World to load (defaults to this server's configured default)"),
          {
            type: BOOLEAN,
            name: 'private',
      ...
      },
      {
        ...
        options: [
          worldOption("World to inspect (defaults to this server's default world)"),
        ],
      },
      {

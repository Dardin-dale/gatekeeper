...
try { process.loadEnvFile(); } catch (e) { /* .env optional when using config/<game>.discord.json */ }

// Per-game Discord app credentials: prefer the gitignored
// config/<game>.discord.json ({ appId, publicKey, botToken }), fall back to .env.
function loadDiscordCreds() {
  const game = process.env.GAME || 'abiotic-factor';
  const cfgPath = path.join(__dirname, 'config', `${game}.discord.json`);
  let cfg = {};
  if (fs.existsSync(cfgPath)) {
    cfg = JSON.parse(fs.readFileSync(cfgPath, 'utf-8'));
    console.log(`Using Discord app from config/${game}.discord.json`);
  }
  return {
    appId: cfg.appId || process.env.DISCORD_APP_ID,
    botToken: cfg.botToken || process.env.DISCORD_BOT_SECRET_TOKEN,
  };
}
...
async function registerCommands() {
  ...
  const { appId, botToken } = loadDiscordCreds();
